...
bool CutoffNeighborFinder::prepare(double cutoffRadius, ParticleProperty* positions, const SimulationCell& cellData){
    ...
    if(_cutoffRadius <= 0.0){
        throw std::runtime_error("Invalid parameter: Neighbor cutoff radius must be positive.");
    }

    simCell = cellData;
    if(simCell.volume3D () <= EPSILON){
        throw std::runtime_error("Invalid input data: Simulation cell is degenerate.");
    }
    ...
    for(size_t i = 0; i < 3; i++){
        ...
        double x = std::abs(simCell.matrix().column(i).dot(planeNormals[i]) / _cutoffRadius);
	...
	auto shortestCellCellDistance = [binCell, planeNormals](const Vector3I& d) {
        ...
        for(size_t dim = 0; dim < 3; dim++){
            ...
            assert(std::abs(n.squaredLength() - 1.0) < EPSILON);
            t = n.dot(p);
            Vector3 p0 = p - t * n;
			double a = u.dot(v)*p0.dot(v) - v.squaredLength()*p0.dot(u);
			double b = u.dot(v)*p0.dot(u) - u.squaredLength()*p0.dot(v);
			double denom = u.dot(v);
			denom *= denom;
        ...
        for(int ix = -stencilRadius; ix <= stencilRadius; ix++){
			for(int iy = -stencilRadius; iy <= stencilRadius; iy++){
				for(int iz = -stencilRadius; iz <= stencilRadius; iz++){
                    ...
                    double shortestDistance = DOUBLE_MAX;
    ...
    for(;;){
        ...
        for(;;){
            ...
            for(size_t k = 0; k < 3; k++){
                ...
                if(!_builder.simCell.pbcFlags()[k]){
                    ...
                    if(_currentBin[k] >= _builder.binDim[k]){
                        ...
                        if(s > std::numeric_limits<int8_t>::max()){
                            throw std::runtime_error("Periodic simulation cell is too small or cutoff radius is too large to generate neighbor lists.");
                        }
						_pbcShift[k] = (int8_t)s;
						_currentBin[k] -= s * _builder.binDim[k];
						_shiftedCenter -= _builder.simCell.matrix().column(k) * (double)s;
                ...
                assert(_currentBin[k] >= 0 && _currentBin[k] < _builder.binDim[k]);
            }
